class Categorys::CategoryBase < ActiveRecord::Base
  # RECODE_FILE_PREFIX  = 'unknown_building_format'
  RECODE_FILE_PATH    = 'recode'
  # RECODE_START_STRING =  'unknow building format info below : '
  ...
  def self.all_types
    self.all.map{ |type| {category: type.category, id: type.id} }
  end
  ...
  def self.recode_start_strgin
    'unknow building format info below : '
  end
end
